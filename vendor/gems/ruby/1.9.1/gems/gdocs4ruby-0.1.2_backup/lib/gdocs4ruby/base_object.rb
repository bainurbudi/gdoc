    ...
    def save
      if @exists
        if (not @local_file.nil? and @local_file.is_a? String) or @content
          @include_etag = false
          if @local_file
            ret = service.send_request(GData4Ruby::Request.new(:put, @edit_content_uri,
                create_multipart_message([{:type => 'application/atom+xml', :content => to_xml()}, {:type => UPLOAD_TYPES[File.extname(@local_file).gsub(".", "").to_sym], :content => get_file(@local_file).read}]), {'Content-Type' => "multipart/related; boundary=#{BOUNDARY}", 'Content-Length' => File.size(@local_file).to_s, 'Slug' => File.basename(@local_file), 'If-Match' => "*"}))
          elsif @content
...
#    end
    ...
    def create
      ret = if ((not @local_file.nil? and @local_file.is_a? String) or @content) and 1 == 0
        if @local_file
          service.send_request(GData4Ruby::Request.new(:post, DOCUMENT_LIST_FEED,
              create_multipart_message(
                [{:type => 'application/atom+xml', :content => to_xml()},
                  {:type => UPLOAD_TYPES[File.extname(@local_file).gsub(".", "").to_sym],
                    :content => get_file(@local_file).read}]),
              {'Content-Type' => "multipart/related; boundary=#{BOUNDARY}",
                'Content-Length' => File.size(@local_file).to_s, 'Slug' => File.basename(@local_file)}))
        elsif @content
...
#</entry>
#        '
#        service.send_request(GData4Ruby::Request.new(:post,
#            'https://docs.google.com/feeds/upload/create-session/default/private/full/document%3A1_Z4-VN1qLdhPQz-cUPHGhlNSFlhWobaD81Lwk_3tX-8',
#            mm))
        mm = '
<?xml version="1.0" encoding="UTF-8"?>
<entry xmlns="http://www.w3.org/2005/Atom" xmlns:docs="http://schemas.google.com/docs/2007">
  ...
  <title>Legal Contract</title>
        ...
        '
        service.gdata_version = "3.0"
        service.send_request(GData4Ruby::Request.new(:post,
            'https://docs.google.com/feeds/upload/create-session/default/private/full/document%3A1_Z4-VN1qLdhPQz-cUPHGhlNSFlhWobaD81Lwk_3tX-8',
            create_multipart_message([{:type => 'application/atom+xml',
                  :content => mm}, 
                {:content => File.open('public/seed.sql','rb').read }]), {'Content-Type' => "multipart/related; boundary=#{BOUNDARY}", 'Content-Length' => "100"}))
     end
