...
#acl_entry = "<?xml version='1.0' encoding='UTF-8'?><entry xmlns='http://www.w3.org/2005/Atom' xmlns:docs='http://schemas.google.com/docs/2007'><!-- Replace the following line appropriately to create another type of resource. --><category scheme='http://schemas.google.com/g/2005#kind' term='http://schemas.google.com/docs/2007#document'/><title>Legal Contract</title></entry>"
acl_entry = "<?xml version='1.0' encoding='UTF-8'?><entry xmlns='http://www.w3.org/2005/Atom' xmlns:docs='http://schemas.google.com/docs/2007'>  <title>Legal Contract</title></entry>"

       pp "-------------------"
        #pp acl_entry

        pp "sassssssssssssssssssssssssssssssssss"
        pp y create_multipart_message([{:type => 'application/atom+xml',
                  :content => acl_entry, :x_content => 'application/pdf', :x_length => 1073741824}] )
        #service.send_request(GData4Ruby::Request.new(:post, DOCUMENT_LIST_FEED, acl_entry))
        service.send_request(GData4Ruby::Request.new(:post, DOCUMENT_LIST_FEED, create_multipart_message([
                {:type => '0',
                  :content => acl_entry, :x_content => 'application/pdf', :x_length => 100}] )))
      end
      return ret
    end
...
#    end
    def create_multipart_message(parts)
      ret = ''
      parts.each do |p|
        ret += "--#{BOUNDARY}\nContent-Length:#{p[:type]}\n\n X-Upload-Content-Type:#{p[:x_content]}\n\n X-Upload-Content-Length:#{p[:x_length]}\n\n#{p[:content]}"
      end
